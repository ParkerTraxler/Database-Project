...
const EditManagerAccount = () => {
    ...
    useEffect(()=>{
        const fetchAccount = async ()=>{
            try{
                const res = await axios.get(`http://mfa-backend-chh3dph8gjbtd2h5.canadacentral-01.azurewebsites.net/manager/${encodeURIComponent(email)}`, // call api endpoint thingy
                    {
                        headers: {
    ...
    const handleClick = async e =>{ 
        ...
        try{
            const res = await axios.put(`http://mfa-backend-chh3dph8gjbtd2h5.canadacentral-01.azurewebsites.net/manager`, {
